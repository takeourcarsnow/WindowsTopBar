...
impl WeatherCondition {
    ...
    fn from_wwo_code(code: u32) -> Self {
        match code {
            ...
            179 | 182 | 185 | 227 | 230 | 281 | 284 | 311 | 314 | 317 | 320 | 323 | 326 | 329 | 332 | 335 | 338 | 350 | 353 | 362 | 365 | 368 | 371 | 374 | 377 => Self::Snow,
            _ => Self::Unknown,
        }
    }
}
...
impl WeatherModule {
    ...
    fn parse_wttr_response(json: &str) -> Result<WeatherData, String> {
        ...
        let current = parsed.get("current_condition")
            .and_then(|c| c.as_array())
            .and_then(|arr| arr.first())
            .ok_or("Missing current_condition")?;

        // Get location info
        let nearest_area = parsed.get("nearest_area")
            .and_then(|n| n.as_array())
            .and_then(|arr| arr.first())
            .ok_or("Missing nearest_area")?;

        let area_name = nearest_area.get("areaName")
        ...
        let country = nearest_area.get("country")
            .and_then(|a| a.as_array())
            .and_then(|arr| arr.first())
            .and_then(|v| v.get("value"))
            .and_then(|v| v.as_str())
        ...
        let weather_code = current.get("weatherCode")
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse::<u32>().ok())
            .unwrap_or(0);

        let description = current.get("weatherDesc")
        ...
        let weather = parsed.get("weather")
            .and_then(|w| w.as_array())
            .and_then(|arr| arr.first());

        let (high, low) = if let Some(today) = weather {
            let h = today.get("maxtempC")
                .and_then(|v| v.as_str())
                .and_then(|s| s.parse::<f32>().ok())
                .unwrap_or(temp_c);
            let l = today.get("mintempC")
                .and_then(|v| v.as_str())
                .and_then(|s| s.parse::<f32>().ok())
                .unwrap_or(temp_c);
            (h, l)
